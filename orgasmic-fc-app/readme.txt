...
* Pending queue rows with the same user+tag are updated instead of sending duplicates
* FCM uses collapse keys so Android/iOS replace the previous banner
* Event hosts get a stacked push when people RSVP “dabei”

= 1.1.28 =
* iOS native shell uses Human Interface-style sheets, switches, 44pt targets, and blur chrome
* Notification prefs auto-save; in-app links for privacy policy, child safety, and content reports
...
= 1.1.22 =
* Device logs use XHR (Android WebView drops sendBeacon); overlay shows real send errors
* Snapshot includes feed/load counts, visible text, and failed requests
