...
= 1.2.21 =
* Define the missing Bunny collection option so WP-Admin no longer fatal-errors

= 1.2.20 =
* Click-to-play uses Bunny's CDN thumbnail.jpg (no player). Resolves the Stream pull-zone host via oEmbed.

= 1.2.19 =
* Click-to-play posters load a real Bunny thumbnail via WordPress (no empty black boxes)
...
= 1.2.16 =
* Composer video player has a remove control so the clip can be taken out of the draft
...
* Strip player.mediadelivery.net URLs from Angesagte Beiträge; keep only the Video chip there
